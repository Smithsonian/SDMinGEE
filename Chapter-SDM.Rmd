...
You can access the GEE repository with the code of the following examples at:
...
![Figure S1. Uploading assets to Google Earth Engine. 1) Click new and then CSV. 2) Click upload. 3)Select the file from your computer. 4) Provide name for asset and the names of the columns containing coordinates in degrees.](./Figures/Fig1.jpg)
...
![Figure S2. Example on presence point rarification. A) Original dataset; B) Final dataset with only one point per pixel.](./Figures/Fig2.png)
...
To find spatial data sets, you can use the search bar. All information related to each spatial data set is available by clicking on the name of the product. The code necessary to import the dataset is available as shown in the following figure. 
...
Note that the `ee.Algorithms.Terrain()` allows you to calculate slope, aspect, and hillshade from the elevation dataset.
...
![Figure S4. Presence data for Bradypus variegatus. Data was obtained from GBIF.](./Figures/Fig4.png)
...
> Note in the code we are using right and left instead of the default Map statement, given that we divided the interactive map in two elements, right and left.

![Figure S5. This figure shows the area of interest defined as a 100 km buffer around the bonding box containing all presence locations.](./Figures/Fig5.png)
...
![Figure S6. Example of predictor variables. A) Elevation; B) Temperature seasonality; C) Annual precipitation; D) Median percentage of tree cover between 2003 and 2020.](./Figures/Fig6.png)
...
> This is a function that requires high memory to GEE as it is working with a large feature collection. Once you have run the correlation and defined the set of covariables to use, it is recommended to comment out the print function so the correlation matrix is not printed out to the console. 
...
![Figure S7. Example of buffer zone used to create random pseudo-absences. The enlarged box shows how pixels with presence locations are masked to avoid creating pseudoabsences in the same location of presences.](./Figures/Fig7.png)
...
![Figure S8. Example of two spatial blocks created for cross validation. In the two examples shown here, blocks of 500 x 500 km have been randomly split in 70% for training data (blue) and 30% for validation data (red).](./Figures/Fig8.png)
...
The first function allows to create random seeds for randomly splitting blocks and generating pseudo-absences at each iteration. 
...
It is important to check that you have a sufficient number of points for model validation at each run. Because the final number of points depend on the random split of blocks, you want to make sure there are enough presence and pseudo-absence points for model validation. 
...
// Here an example to export the training and validation data sets from the first iteration. 
...
For this analysis we will define a study area encompassing Costa Rica and Panama countries. For replicability, we manually define a polygon geometry.
...
![Figure S11. Defined area of interest to study *Cebus capucinus* habitat suitability change.](./Figures/Fig11.png)
...
![Figure S16. Regression slope showing a grade from positive (blue) to negative (red) trends in habitat suitability change between 2000 and 2019 for *Cebus capucinus*. Only pixels with significative trends are shown.](./Figures/Fig16.png)
