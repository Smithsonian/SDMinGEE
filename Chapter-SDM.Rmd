...
This method requires fitting a cluster function to the occurrence data. When you have a large presence dataset, you can randomly choose a subset of the data with this line of code: `Data.randomColumn().sort('random').limit(200)` where the number in `limit()` specifies the number of random presence used to train the clusterer. A critical step is to display the cluster results and identify the correct cluster, zero or one, to use for creation of pseudo-absences. The cluster ID is assigned in a non consistent manner and can change even when changing the order of the same data input. We wrote a line of code to automatically identify the cluster ID to define the area to create pseudo-absences. But it is important to display the cluster results and confirm that the cluster worked properly and that the correct cluster ID was identified before creating the mask: `var mask2 = Clresult.select(['cluster']).eq(clustID)`.
...
// Obtain the ID of the cluster similar to data and use the oposite to define the area to create pseudo-absences.
...
// Display cluster results and identify the cluster ID similar and dissimilar to presence data
right.addLayer(Clresult.randomVisualizer(), {}, 'Clusters', 0);
// Mask cluster that is dissimilar to presence data.
// Obtain the ID of the cluster similar to data and use the opposite to define the area to create pseudo-absences.
var clustID = Clresult.sampleRegions({collection: Data.randomColumn().sort('random').limit(200), properties: [], tileScale: 16, scale: GrainSize});
clustID = ee.FeatureCollection(clustID).reduceColumns(ee.Reducer.mode(),['cluster']);
clustID = ee.Number(clustID.get('mode')).subtract(1).abs();
var mask2 = Clresult.select(['cluster']).eq(clustID);
...
function makeGrid(geometry, scale) {
  ...
  return lonGrid
    .multiply(latGrid)
    .reduceToVectors({
      geometry: geometry.buffer({distance:20000,maxError:1000}), //The buffer allows you to make sure the grid inclused the borders of the AOI.
      scale: scale,
...
# Case Study 3: Modeling species distribution at high spatial resolution using unclassified satellite images as predictor variables

In this third case study, we demonstrate the implementation of the SDM workflow combining presence records and unclassified satellite data. The code workflow is very similar to case study 1, with the difference in the predictor variables used and the fact that this analysis has to be run on batch mode to avoid memory limits. Therefore, all results has to be exported to Google Drive. Note that it is possible to run a couple of iterations to quickly visualize results on the interactive map before exporting the model with several iterations.

## Loading data and defining grid size and extent

For this example, we obtained the eBird observation dataset for *Hylocichla mustelina* from GBIF (GBIF.org (12 November 2021); https://doi.org/10.15468/dl.hpjfup) for the month of June (middle of breeding season when observation of migrants is more unlikely) for the years 2018, 2019 and 2020. The dataset was first ingested as an asset.

In the first section we load the data, we set the spatial resolution of the analysis to 90 m. 

We defined the extent of the analysis to the eastern continental USA (4,606,284 km2), limiting the extent to the western observation in the dataset (longitude: -103 degrees). To do this we used the Large Scale International Boundary (LSIB) dataset and filtered the USA. We used the `intersection()` function to limit the geometry to the -103 degrees of longitude.

We rarified the original 99,939 observations to keep one per pixel, resulting in 34,880 observations for modeling. 
...
In the next step we select the predictor variables. 

We modeled *Hylocichla mustelina* habitat suitability using atmospherically corrected Landsat 8 surface reflectance (SR) collection 2, Synthetic Aperture Radar (SAR) and temperature products as predictor variables. 
...
We finally created a mosaic of the entire study area calculating the median value for each pixel for all Landsat image bands. 
...
You need to display the resulting images to make sure you have a complete mosaic for the study area with minimal gaps do to clouds or bad quality pixels. We had to modify the filter parameters until the complete Lansdat mosaic was obtained.
...
For this analysis we implemented a two-step environmental profiling technique. We used a cluster with Euclidean distance on 1,000 random selected occurrences to restrict the area for the creation of pseudo-absences to pixels more dissimilar to the environmental profile of the occurrence data.
...
For the analysis, we generated an equal number of pseudo-absences as occurrence data for each of the 5 datasets. To ensure the equal number of pseudo-absences, we hard coded 50000 random points as likely, many will be created on masked pixels of the extent of the random blocks and discarded. Hence, to ensure same number of pseudo-absence than presence points, we first create 50000 and then limit to the equal number of presence. We fit a random forest model (500 trees) to each individual training dataset and then averaged model outputs to calculate the mean habitat suitability of the five iterations. 
...
We assessed model accuracy calculating the AUC-PR, sensitivity, and specificity on the validation dataset of each model iteration. To obtain a binary potential distribution map we used the mean threshold that maximized the sum of sensitivity and specificity of each of the individual model predictions. 
...
Because exporting results for this analysis take a long period of time, instead of directly exporting the binary presence-absence map, we manually calculated the average threshold from the exported metrics and set the threshold on the visualization parameters in QGIS.
