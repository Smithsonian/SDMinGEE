---
title: "Supplemental Materials - Apendix 1 - Implementation of ensemble species distribution models in Google Earth Engine"
...
![Figure S1. Uploading assets to Google Earth Engine. 1) Click new and then CSV. 2) Click upload. 3)Select the file from your computer. 4) Provide name for asset and the names of the columns containing coordinates in degrees.](Fig1.jpg)
...
One important step in modeling species distributions is to avoid the potential effect of data aggregation resulting from multiple nearby observations. This reduces the impact of sampling bias on the model. 
...
![Figure S2. Example on presence point rarification. A) Original dataset; B) Final dataset with only one point per pixel.](Fig2.png)
...
![Figure S3. SRTM Digital Elevation Data description with code to import the dataset](Fig3.JPG)
...
# Case Study 1: Modeling *Bradypus variegatus* habitat suitability and distribution using presence-only data

> The entire code for this example is available at: https://code.earthengine.google.com/a8d81fa8f380315f716aad1ae1437059
...
// Add two maps to the screen so we can visualize several outputs.
var left = ui.Map(); // Map to the left
var right = ui.Map(); // Map to the right
ui.root.clear();
ui.root.add(left);
ui.root.add(right);
...
![Figure S4. Presence data for Bradypus variegatus. Data was obtained from GBIF and cleaned manually using QGIS software.](Fig4.png)
...
![Figure S5. This figure shows the area of interest defined as a 100 km buffer around the bonding box containing all presence locations.](Fig5.png)
...
// Display layers on the right map
...
![Figure S6. Example of predictor variables. A) Elevation; B) Temperature seasonality; C) Annual precipitation; D) Median percentage of tree cover between 2003 and 2020.](Fig6.png)

> Note that in the code, we generally comment out lines of code that print objects to the console or add layers to the map. This is becuase with large and complex codes GEE is likely to reach memory cuotas for users and give memory related errors. A good practice to avoid such errors is to minimize the number of objects being printed or displayed. In the early stages of the model, you can print and visualize objects. Once you are happy with the opperations, you can comment out those lines of code to avoid problems with later more advanced functions for fitting models. This will largely depend on the extent of your analysis, the number of predictor variables being procesed, the number of model iterations, and the total number of precense data. 

It is important to make sure that there is no significant correlation among predictor variables that can cause collinearity effects. We account for this by estimating the Spearman correlation among predictor variable values at 5000 randm locations. Highly correlated predictor variables should not be included in the same model.
...
> This is a function that requieres high memory to GEE as it is working with a large feature collection. Once you have run the correlation and defined the set of covariables to use, it is recommended to comment out the print function so the correlatin matrix is not printed out to the console. 
...
//right.addLayer(AreaForPA)
```

![Figure S7. Example of buffer zone used to create random pseudoabsences. The enlarged box shows how pixels with presence locations are masked to avoid creating pseudoabsences in the same location of presences.](Fig7.png)

We implemented a block repeated split-sample cross-validation technique to randomly partition data for model training and validation (Roberts et al., 2017; Valavi, Elith, Lahoz-Monfort, & Guillera-Arroita, 2019). We will then run multiple model iterations with random block splits to create training and validation datasets.
...
function makeGrid(geometry, scale) {
  ...
  return lonGrid
    .multiply(latGrid)
    .reduceToVectors({
      geometry: geometry.buffer(1000000), //Buffer to expand grid and include borders
      scale: scale,
...
![Figure S8. Example of two spatial blocks used to fold randomly for cross validation. In the two examples, blocks have been randomly splet in 70% for training data (blue) and 30% for validation data (red).](Fig8.png)
...
There are several non-parametric classifier algorithms available in GEE that can be easily implemented. These include, Random Forest, support vector machine, classification and regression trees, maximum entropy, and gradient boosting.

We implement a 10 block repeated split-sample cross-validation technique. The number of pseudo-absences is balanced with the number of presences to avoid problems in model fitting with gradient boosting or Random Forest classifiers (Evans et al., 2011; Barbet-Massin et al. 2012). In this case, we will use Random Forest. But note that in the `SDM` function, we also provide the code to fit gradient boosting classifiers. To change the classifier, you need to comment out the Random Forest using two forward slashes (`//`) and activate the gradient boosting classifier. 
...
Now we need to define some parameters for the model. A variable with the number of presence points, the percentage for data split, and the number of iterations.
...
We can now map the model function. Instead of generating random numbers, we will manually set 10 random numbers for reproducibility of our results. The length of the list determines the number of iterations.
...
![Figure S9. Estimated ensemble habitat suitability model for *Bradypus variegatus*.](Fig9.png)
...
![Figure S10. Final estimated distribution model for *Bradypus variegatus*.](Fig10.png)
...
![Figure 10. Defined area of interest to study *Cebus capucinus* suitable habitat change.](Fig11.png)
...
![Figure 11.*Cebus capucinus* presence locations.](Fig12.png)
...
![Figure 12.2019 presence data used for model validation.](Fig13.png)
...
![Figure 13.Predicted *Cebus capucinus* habitat suitability for 2003.](Fig14.png)

![Figure 14.Predicted *Cebus capucinus* habitat suitability for 2011.](Fig15.png)

![Figure 15.Predicted *Cebus capucinus* habitat suitability for 2019.](Fig16.png)
...
![Figure 16.Regression slope showing a grade from positive (green), to neutral (yellow), to negative (red) trends in habitat suitability change between 2000 and 2019 for *Cebus capucinus*.](Fig17.png)
...
![Figure 17.*Cebus capucinus* habitat suitability change across years.](Fig18.gif)
